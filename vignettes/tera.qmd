...
To get a feel for what `rtera` can do, let's start with a simple "hello world"
example.
...
Everything in `rtera` revolves around the `rtera` object, which serves as a 
template library with encapsulated rendering methods. In the above example, we 
initialize an `rtera` with an empty template library by calling `Tera$new()` 
with no arguments.
...
initialize an `rtera` by specifying the directory with a glob pattern. Suppose, 
for example, that you have a website directory that looks like this: 

```{r}
#| label: directory
template_dir <- system.file("templates", package = "rtera")
...
You can generate a new `rtera` around this directory like so
...
tera$add_file_templates(
  "base.html" = file.path(template_dir, "hello.html"),
...
```{r}
#| label: reload
example_dir <- file.path(tempdir(), "rtera-templates")
...
content blocks where content from a child document should be injected. For
instance, our current library has `base.html`:
